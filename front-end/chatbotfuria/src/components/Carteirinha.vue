    ...
    methods: {
      async checkExistingEntry() {
        try {
          ...
          const response = await this.makeAuthenticatedRequest(
            `${import.meta.env.VITE_API_BASE}/api/v1/quiz/check-entry/`
          );
          ...
          if (data.exists) {
            ...
            try {
              const twitterResponse = await this.makeAuthenticatedRequest(
                `${import.meta.env.VITE_API_BASE}/api/v1/quiz/auth/twitter/status/`
              );
      ...
      async handleTokenRefresh() {
        try {
          ...
          const response = await fetch(`${import.meta.env.VITE_API_BASE}/chat/refresh/`, {
            method: "POST",
      ...
      async loadChatHistory() {
        try {
          const response = await this.makeAuthenticatedRequest(
            `${import.meta.env.VITE_API_BASE}/chat/history-count/`
          );
      ...
      try {
        ...
        const response = await this.makeAuthenticatedRequest(
          `${import.meta.env.VITE_API_BASE}/api/v1/quiz/formulario/`, 
          {
            method: 'POST',
            body: formData
          }
      ...
      async submitUpdate() {
        try {
          ...
          const response = await this.makeAuthenticatedRequest(
            `${import.meta.env.VITE_API_BASE}/api/v1/quiz/update-entry/`,
            {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
      ...
      async updateTwitterData() {
        try {
          ...
          const response = await this.makeAuthenticatedRequest(
            `${import.meta.env.VITE_API_BASE}/api/v1/quiz/refresh-validation/`,
            {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' }
            }
      ...
      async linkTwitterAccount() {
        ...
        try {
          ...
          const response = await this.makeAuthenticatedRequest(
            `${import.meta.env.VITE_API_BASE}/api/v1/quiz/auth/twitter/start/`
          );
      ...
      openTwitterAuthPopup(url) {
          ...
          const messageHandler = (event) => {
            if (event.origin !== `${import.meta.env.DEFAULT_URL}`) return;
